...
class CredibleLoginCredentials(AlgObject):
    ...
    def parse_json(cls, json_dict):
        cookie_values = json_dict['cookie_values']
        cookie_jar = requests.utils.cookiejar_from_dict(cookie_values)
        authenticated_session = requests.Session()
        authenticated_session.cookies = cookie_jar
        return cls(
            json_dict['id_source'], json_dict['domain_name'], json_dict['site_url'],
            authenticated_session, json_dict['time_generated']
        )

    @property
    def to_json(self):
        cookie_jar = self._authenticated_session.cookies
        cookie_values = requests.utils.dict_from_cookiejar(cookie_jar)
        return {
            'id_source': self._id_source,
            'domain_name': self._domain_name,
            'site_url': self._site_url,
            'time_generated': self._time_generated,
